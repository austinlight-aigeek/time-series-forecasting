...
desired_output = sigma*train_data(2:end)+mu;
h_train = figure(1);
...
for epoch = 1:epoch_num
    ...
    for p = 1:numMiniBatch
        ...
        for i = 1:miniBatchSize
            s = bStart + (i-1)*m;
            input = [input train_data(s:s+m-1)'];
            target = [target train_data(s+m:s+2*m-1)'];
        end
...
save('Parameters.mat', 'Wf', 'Wi', 'Wg', 'Wo',...
    'Rf', 'Ri', 'Rg', 'Ro', 'bf', 'bi', 'bg', 'bo','V','b', ...
    'l','m','n', 'numMiniBatch', 'miniBatchSize', 'train_data', 'test_data',...
    'sigma', 'mu');
