...
m = 1;                    % input dimension
l = 250;                   % hidden node dimension
n = 1;                    % target dimension
miniBatchSize = 50;       % length of LSTM network
...
mu = mean(data); sig = std(data);

std_train_data = (train_data - mu) / sig;
std_test_data = (test_data - mu) / sig;

denoised_std_train_data = wdenoise(std_train_data,3,'Wavelet','db4',...
                      'DenoisingMethod','SURE');

% plot(std_train_data,'r-')
% hold on
% plot(denoised_std_train_data,'b-')
